...
# (1) w
sw <- w_inference(chains=list(sims1, sims2, sims3, sims4), B=B, p=p, n.sim=n.sim, warmup=n.sim/2+1, thin=1, filename="PD2_Musc_w_trace.pdf", plot=F, d=3)
sw$mon.w                    # all converged
w_hat_2S_Gibbs <- sw$w.mode
sw_mat <-rbind(sw$sw[,1,], sw$sw[,2,], sw$sw[,3,], sw$sw[,4,])


tiff("Fig9s.tiff", height=10, width=10, res=300, units="in")
par(mfrow=c(3, 3))
for(j in 1:9) {
...
tiff("Fig10s.tiff", width=10, height=10, res=300, units="in")
...
# (2) v
sv <- v_inference(chains=list(sims1, sims2, sims3, sims4), B=B, p=p, N=N, n.sim=n.sim, warmup=n.sim/2+1, thin=1, filename="PD2_Musc_v_trace.pdf", plot=F, d=3)
...
range(R_hat)  #  0.9922876 1.0264612


# save image for later use
save.image("PD2_Musc_fit.Rdata")
